...
def main() -> int:
    ...
    foundation_api_p = sub.add_parser(
        "foundation-api",
        help="Run the FOUNDATION API scaffold (auth/onboarding/memory)",
    )
    foundation_api_p.add_argument("--host", default="127.0.0.1", help="Bind host")
    foundation_api_p.add_argument("--port", type=int, default=8797, help="Bind port")
    foundation_api_p.add_argument("--debug", action="store_true", help="Enable Flask debug mode")
    foundation_api_p.set_defaults(
        func=lambda args: _run(
            [
                sys.executable,
                os.path.join(BASE_DIR, "scripts", "foundation_api.py"),
                "--host",
                args.host,
                "--port",
                str(args.port),
