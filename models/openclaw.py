"""
models/openclaw.py - OpenClaw adapter
CANON: Provider-specific HTTP calls stay inside model adapters.

This integration assumes an OpenAI-compatible chat completions surface.
Override the base URL and model ids with env vars if your OpenClaw deployment differs.
"""
...
DEFAULT_BASE_URL = "https://api.openclaw.ai/v1"
...
class OpenClawModel(BaseModel):
    MODELS = {
        "opus": "openclaw-opus",
        "sonnet": "openclaw-sonnet",
        "haiku": "openclaw-haiku",
    }

    ENV_BY_TIER = {
        "opus": "PERMANENCE_OPENCLAW_MODEL_OPUS",
        "sonnet": "PERMANENCE_OPENCLAW_MODEL_SONNET",
        "haiku": "PERMANENCE_OPENCLAW_MODEL_HAIKU",
    }
    ...
    def __init__(self, tier: str = "sonnet", model_id: str | None = None):
        ...
        self.name = f"openclaw_{self.tier}"
        self.base_url = str(os.getenv("PERMANENCE_OPENCLAW_BASE_URL", DEFAULT_BASE_URL)).rstrip("/")

        api_key = str(os.getenv("OPENCLAW_API_KEY", "")).strip()
        if not api_key or api_key in {"your_key_here", "YOUR_KEY_HERE"}:
            raise RuntimeError("OPENCLAW_API_KEY not set. Add to .env file.")
    ...
    def generate(self, prompt: str, system: str = None) -> ModelResponse:
        ...
        if response.status_code >= 400:
            ...
            raise RuntimeError(f"OpenClaw API error {response.status_code}: {detail}")

        try:
            body = response.json()
        except ValueError as exc:
            raise RuntimeError(f"OpenClaw API returned invalid JSON: {exc}") from exc

        choices = body.get("choices")
        if not isinstance(choices, list) or not choices:
            raise RuntimeError("OpenClaw API returned no choices")
        first = choices[0] if isinstance(choices[0], dict) else {}
        message = first.get("message") if isinstance(first, dict) else {}
        text = _string_content((message or {}).get("content"))
        if not text:
            raise RuntimeError("OpenClaw API returned empty response text")
