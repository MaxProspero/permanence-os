"""
models/xai.py - xAI (Grok) adapter
...
class XAIModel(BaseModel):
    ...
    ENV_BY_TIER = {
        "opus": "PERMANENCE_XAI_MODEL_OPUS",
        "sonnet": "PERMANENCE_XAI_MODEL_SONNET",
        "haiku": "PERMANENCE_XAI_MODEL_HAIKU",
    }
    ...
    def __init__(self, tier: str = "sonnet", model_id: str | None = None):
        ...
        self.name = f"xai_{self.tier}"
        self.base_url = str(os.getenv("PERMANENCE_XAI_BASE_URL", DEFAULT_BASE_URL)).rstrip("/")

        api_key = str(os.getenv("XAI_API_KEY", "")).strip()
        if not api_key:
            raise RuntimeError("XAI_API_KEY not set. Add to .env file.")
    ...
    def generate(self, prompt: str, system: str = None) -> ModelResponse:
        ...
        if response.status_code >= 400:
            ...
            raise RuntimeError(f"xAI API error {response.status_code}: {detail}")

        try:
            body = response.json()
        except ValueError as exc:
            raise RuntimeError(f"xAI API returned invalid JSON: {exc}") from exc

        choices = body.get("choices")
        if not isinstance(choices, list) or not choices:
            raise RuntimeError("xAI API returned no choices")
        first = choices[0] if isinstance(choices[0], dict) else {}
        message = first.get("message") if isinstance(first, dict) else {}
        text = _string_content((message or {}).get("content"))
        if not text:
            raise RuntimeError("xAI API returned empty response text")
    ...
    def is_available(self) -> bool:
        api_key = str(os.getenv("XAI_API_KEY", "")).strip()
        return bool(REQUESTS_AVAILABLE and api_key)
